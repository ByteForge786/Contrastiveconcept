def sample_pairs_batch(self,
                        ...
                        batch_size: int = None) -> SamplingResult:
        ...
        try:
            ...
            n_hard = int(0.4 * n_required)  # 40% hard negatives
    ...
    def _sample_medium_negatives(self,
                              domain: str,
                              concept: str,
                              n_required: int,
                              attributes_df: pd.DataFrame) -> List[Tuple[str, str, float]]:
        """Sample medium difficulty negatives."""
        try:
            medium_negatives = []
            
            # Get attributes from different domains or concepts
            ...
            if len(potential_negatives) < n_required:
                self.logger.warning(f"Not enough potential negatives for {domain}-{concept}")
                n_required = len(potential_negatives)
            
            # Randomly sample negatives
            sampled_indices = np.random.choice(
                len(potential_negatives),
                size=min(n_required * 2, len(potential_negatives)),  # Sample extra for filtering
                replace=False
            )
            ...
            for idx in sampled_indices:
                ...
                # Use constant weight for medium negatives
            ...
            self.logger.error(f"Error sampling medium negatives: {str(e)}")
            return []
    ...
    def clear_cache(self):
        ...
        self.logger.info("Cleared similarity and batch caches")import torch
...
class OptimizedDataProcessor:
    ...
    def get_attribute_text(self, row: pd.Series) -> str:
        """Get combined attribute text."""
        return f"{row['attribute_name']} {row['description']}"

    def get_concept_definition(self, domain: str, concept: str) -> str:
        """Get concept definition with caching."""
        cache_key = (domain, concept)
        if cache_key in self.domain_concepts:
            return self.domain_concepts[cache_key]
        ...
        self.domain_concepts[cache_key] = definition
        return definition
